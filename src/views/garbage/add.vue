...
export default {
  ...
  mounted() {
    ...
    if (params != null&&params._id != undefined&&params._id != "" ) {
      this.ruleForm = params;
      this.submitBtn = "更新垃圾名称";
    }
  },
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (this.ruleForm._id == "" || this.ruleForm._id == undefined) {
            this.$store
              .dispatch("user/invokeInsertGabarge", this.ruleForm)
              .then(response => {
